...
class model_student extends CI_Model {
    ...
    public function getSelectedPrimaryPayment($id, $education_id, $period_id){
        $query = $this->db->query('SELECT (select student_id from student where student_id = "'.$id.'") as "student_id", (select nis from student where student_id = "'.$id.'") as "nis", (select name from student where student_id = "'.$id.'") as "student_detail", sum(PAYMENT_TYPE.total) as "invoice", case when status = "1" then "Primary" else "Other" end as "detail", sum(CASE WHEN (SELECT sum(PAYMENT.TOTAL) FROM PAYMENT WHERE PAYMENT.PAYMENT_TYPE_ID = PAYMENT_TYPE.PAYMENT_TYPE_ID AND STUDENT_ID="'.$id.'"  LIMIT 1) IS NULL THEN 0 ELSE (SELECT sum(PAYMENT.TOTAL) FROM PAYMENT WHERE PAYMENT.PAYMENT_TYPE_ID = PAYMENT_TYPE.PAYMENT_TYPE_ID AND STUDENT_ID="'.$id.'" LIMIT 1) END) AS "total_payment", CASE WHEN (SELECT PAYMENT.PAYMENT_DATE FROM PAYMENT WHERE PAYMENT.PAYMENT_TYPE_ID = PAYMENT_TYPE.PAYMENT_TYPE_ID AND STUDENT_ID="'.$id.'" order by PAYMENT.PAYMENT_DATE desc LIMIT 1) IS NULL THEN "-" ELSE (SELECT PAYMENT.PAYMENT_DATE FROM PAYMENT WHERE PAYMENT.PAYMENT_TYPE_ID = PAYMENT_TYPE.PAYMENT_TYPE_ID AND STUDENT_ID="'.$id.'" order by PAYMENT.PAYMENT_DATE desc LIMIT 1) END AS "payment_date", sum((PAYMENT_TYPE.TOTAL - (CASE WHEN (SELECT sum(PAYMENT.TOTAL) FROM PAYMENT WHERE PAYMENT.PAYMENT_TYPE_ID = PAYMENT_TYPE.PAYMENT_TYPE_ID AND STUDENT_ID="'.$id.'"  LIMIT 1) IS NULL THEN 0 ELSE (SELECT sum(PAYMENT.TOTAL) FROM PAYMENT WHERE PAYMENT.PAYMENT_TYPE_ID = PAYMENT_TYPE.PAYMENT_TYPE_ID AND STUDENT_ID="'.$id.'"  LIMIT 1) END)) )AS "remain" FROM PAYMENT_TYPE WHERE PAYMENT_TYPE.status= "1" and PAYMENT_TYPE.education_id = "'.$education_id.'" and PAYMENT_TYPE.period_id = "'.$period_id.'" order by remain desc, payment_date asc');
        return $query->row();        
    }
    
    public function getSelectedPrimaryPayment2($id, $education_id, $period_id){
        $query = $this->db->query('SELECT payment_type.payment_type_id, (select student_id from student where student_id = "'.$id.'") as "student_id", (select nis from student where student_id = "'.$id.'") as "nis", (select name from student where student_id = "'.$id.'") as "student_detail", sum(PAYMENT_TYPE.total) as "invoice", case when status = "1" then "Primary" else "Other" end as "payment_status", sum(CASE WHEN (SELECT sum(PAYMENT.TOTAL) FROM PAYMENT WHERE PAYMENT.PAYMENT_TYPE_ID = PAYMENT_TYPE.PAYMENT_TYPE_ID AND STUDENT_ID="'.$id.'"  LIMIT 1) IS NULL THEN 0 ELSE (SELECT sum(PAYMENT.TOTAL) FROM PAYMENT WHERE PAYMENT.PAYMENT_TYPE_ID = PAYMENT_TYPE.PAYMENT_TYPE_ID AND STUDENT_ID="'.$id.'" LIMIT 1) END) AS "total_payment", CASE WHEN (SELECT PAYMENT.PAYMENT_DATE FROM PAYMENT WHERE PAYMENT.PAYMENT_TYPE_ID = PAYMENT_TYPE.PAYMENT_TYPE_ID AND STUDENT_ID="'.$id.'" order by PAYMENT.PAYMENT_DATE desc LIMIT 1) IS NULL THEN "-" ELSE (SELECT PAYMENT.PAYMENT_DATE FROM PAYMENT WHERE PAYMENT.PAYMENT_TYPE_ID = PAYMENT_TYPE.PAYMENT_TYPE_ID AND STUDENT_ID="'.$id.'" order by PAYMENT.PAYMENT_DATE desc LIMIT 1) END AS "payment_date", sum((PAYMENT_TYPE.TOTAL - (CASE WHEN (SELECT sum(PAYMENT.TOTAL) FROM PAYMENT WHERE PAYMENT.PAYMENT_TYPE_ID = PAYMENT_TYPE.PAYMENT_TYPE_ID AND STUDENT_ID="'.$id.'"  LIMIT 1) IS NULL THEN 0 ELSE (SELECT sum(PAYMENT.TOTAL) FROM PAYMENT WHERE PAYMENT.PAYMENT_TYPE_ID = PAYMENT_TYPE.PAYMENT_TYPE_ID AND STUDENT_ID="'.$id.'"  LIMIT 1) END)) ) AS "remain" FROM PAYMENT_TYPE WHERE PAYMENT_TYPE.status= "1" and PAYMENT_TYPE.education_id = "'.$education_id.'" and PAYMENT_TYPE.period_id = "'.$period_id.'" group by payment_type.payment_type_id order by remain desc, payment_date asc');
        return $query->result();        
    }
