...
function initPaymentDT(){    
    ...
    dt_payment = $('#dt-payment').DataTable({
        ajax : '../l-fis/student/getPayment?student_id='+$('#student_id').html(),
        dom: 'Bfrtip', //B -> Button
        buttons: [
            {
            ...
            {
                text: '<i class="fa fa-th-list"></i> Primary',
                className: 'btn btn-default',
                action: function ( e, dt, node, config ) {                    
            ...
            {
                text: '<i class="fa fa-list"></i> Others',
                className: 'btn btn-default',
                action: function ( e, dt, node, config ) {
            ...
            {
                text: '<i class="fa fa-pencil-square-o"></i> Update Primary',
                className: 'btn btn-default',
                action: function ( e, dt, node, config ) { 
                    var formData = new FormData();        
                    formData.append('student_id', $('#student_id').html());   
                    ajaxPro('POST', '/l-fis/student/getSelectedPrimaryPayment', formData, 'json', false, false, false, false, success, success, null);          
...
function getRevisionPayment(i){
    ...
    form.action = '/l-fis/payment_revision';
    form.method = 'post';
...
function getUpdatePayment(i){
    ...
    ajaxPro('POST', '/l-fis/student/getSelectedPayment', formData, 'json', false, false, false, false, success, success, null);          
    function success(output) {           
...
function inputTypePrimaryPayment(){    
    $('#form-primary-payment').bootstrapValidator({
        ...
        $('#form-primary-payment').submit(function (event) {
            ...
            var formData = new FormData($(this)[0]);                
            formData.append('student_id', $('#student_id').html());
            ajaxPro('POST', '/l-fis/student/getSelectedPrimaryPayment2', formData, 'json', false, false, false, false, success, success, null);          
            function success(output){          
                var d = new Date();
...
function inputTypePayment(){    
    $('#form-payment').bootstrapValidator({
        ...
        $('#form-payment').submit(function (event) {
            ...
            formData.append('payment_id', payment_id);                        
            formData.append('payment_group', payment_group);              
            ajaxPro('POST', '/l-fis/payment/insert', formData, 'html', false, false, false, false, success, success, null);          
            function success(output) {            
                dt_payment.ajax.reload();
